...
import type { AdminKyc as AdminKycType } from "../api/adminKyc.api";

const AdminKyc = () => {
  const [kycs, setKycs] = useState<AdminKycType[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [selectedKyc, setSelectedKyc] = useState<AdminKycType | null>(null);
  ...
  const handleReject = async () => {
    ...
    try {
      await rejectKyc(selectedKyc.id, rejectionReason);
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <div className="rounded-xl bg-white p-6 shadow">
          ...
          <h2 className="mt-2 text-3xl font-bold">{kycs.length}</h2>
        </div>
        ...
        <div className="rounded-xl bg-yellow-50 p-6 shadow">
          ...
          <h2 className="mt-2 text-3xl font-bold text-yellow-700">
            {kycs.filter((kyc) => kyc.status === "PENDING").length}
          </h2>
        </div>
        ...
        <div className="rounded-xl bg-green-50 p-6 shadow">
          ...
          <h2 className="mt-2 text-3xl font-bold text-green-700">
            {kycs.filter((kyc) => kyc.status === "APPROVED").length}
          </h2>
        </div>
      </div>
        ...
        <table className="w-full">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="px-4 py-4 text-left">PAN</th>
            ...
            {kycs.length === 0 ? (
              ...
              kycs.map((kyc) => (
                <tr key={kyc.id} className="border-b">
                  ...
                  <td className="px-4 py-4">{kyc.panNumber}</td>

                  <td className="px-4 py-4">{kyc.city}</td>
          ...
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <p className="font-mono">{selectedKyc.userId}</p>
              </div>

              <div>
                <p className="text-sm text-gray-500">Status</p>

                <p className="font-semibold">{selectedKyc.status}</p>
              </div>
              ...
              <div className="md:col-span-2">
                ...
                <p>
                  ...
                  {selectedKyc.addressLine2 && `, ${selectedKyc.addressLine2}`},{" "}
                  {selectedKyc.city}, {selectedKyc.state}, {selectedKyc.country}{" "}
            ...
            <div className="mt-6">
              ...
              <div className="grid gap-4 md:grid-cols-3">
                {selectedKyc.aadhaarImageUrl && (
                  <a
                    href={selectedKyc.aadhaarImageUrl}
                    target="_blank"
                    rel="noreferrer"
                ...
                {selectedKyc.selfieImageUrl && (
                  <a
                    href={selectedKyc.selfieImageUrl}
                    target="_blank"
                    rel="noreferrer"
                    className="rounded-lg border p-4 text-center text-blue-600 hover:bg-gray-50"
                  >
      ...
      {rejectModal && selectedKyc && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-xl bg-white p-6">
            <h2 className="text-xl font-bold">Reject KYC</h2>
